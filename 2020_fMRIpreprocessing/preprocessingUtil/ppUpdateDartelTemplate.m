...
dataBasePath = fullfile(filesep,'Users','daniela','Data','FunImg');
[folderNamesTmp, foldersTmp] = getSubjectFolderNames(fullfile(dataBasePath,'checked'));
templates = dir(fullfile(foldersTmp{1},'anat','DARTEL','Template*'));
for iTemp = 1:6
    templFiles{iTemp,1} = {fullfile(foldersTmp{1},'anat','DARTEL',templates(iTemp).name)};
end
...
jobs = {fullfile(jobsParentPath,'jobs','updateExistingDartelTemplate_job.mat')};
inputs = cell(8, 1);
